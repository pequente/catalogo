...
function formatDuration(ms: number): string {
  ...
  if (min < 60) {
    if (sec) return `${min} min ${sec} s`;
    return `${min} min`;
  }
  const h = Math.floor(min / 60);
  const remMin = min % 60;
  if (remMin) return `${h} h ${remMin} min`;
  return `${h} h`;
}
...
export function UsabilidadePanel({ stats }: Props) {
  const { usabilidade, negocio } = stats;
  ...
  const hasFunil =
    negocio.funil.waClicks > 0 ||
    negocio.funil.leadsLinked > 0 ||
    negocio.funil.pedidosComCliente > 0;
  ...
  return (
    <div
      className={`dashboard-section dashboard-section--usabilidade ${styles.root}`}
    >
      <DashSection
      ...
      >
        <div className={styles.heroBand}>
          ...
          <KpiCard
            ...
            label="Cliques WhatsApp por sessão"
      ...
      >
        {hasFunil ? (
          ...
          <DashEmpty
            ...
            text="Cliques no WhatsApp e vínculos de contato aparecem aqui."
          />
        )}
      </DashSection>
